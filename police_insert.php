<?php
  ...
  if (isset($_POST['upload'])) {
  	...
  	// Get text
        $police_name = mysqli_real_escape_string($db, $_POST['police_name']);
        $police_details = mysqli_real_escape_string($db, html_entity_decode($_POST['police_details']));
	    ...
	    $target = "uploaded_police_image/".basename($image);
		$image_path = "http://www.coxplorebd.com/uploaded_police_image/".$image ;

	  if(empty($police_name) || empty($police_details) || empty($phone)|| empty($latitude )|| empty($longitude)){
		echo '<script language="javascript">';
		echo 'alert("Some Field is Empty You Must Insert Suitable Data On It !")';
		echo '</script>';
	  }else{
		$sql = "INSERT INTO police (police_name,police_details,phone,latitude,longitude,image,image_path) VALUES ('$police_name','$police_details','$phone','$latitude','$longitude','$image','$image_path')";
		// execute query
      mysqli_query($db, $sql);
...
<div id="content">
  <form method="POST" action="police_insert.php" enctype="multipart/form-data">

    <div class="form-group has-success">
        <label class="col-xs-2 control-label" for="inputSuccess">Police Name</label>
        <div class="col-xs-10">
            <input type="text" id="inputSuccess" class="form-control" placeholder="Type Police Name" name="police_name">
        </div>
    </div>

    <div class="form-group has-success">
        <label class="col-xs-2 control-label" for="inputSuccess">Police Details</label>
        <div class="col-xs-10">
            <textarea id="myeditor" name="police_details" id="myeditor"></textarea>
        </div>
    </div>
...
<script type="text/javascript">
  ...
  <center>
    <div>
      ...
      <button type="submit" class="btn btn-primary" style = "width:40%" onclick="window.location.href='police_list.php'">Police List</button>
    </div>
  </center>