...
ob_start();
  ...
  if (isset($_POST['upload'])) {
  	...
  	// Get text
        $tourist_agent_name = mysqli_real_escape_string($db, $_POST['tourist_agent_name']);
        $tourist_agent_details = mysqli_real_escape_string($db, html_entity_decode($_POST['tourist_agent_details']));
	    ...
	    $target = "uploaded_tourist_agent_image/".basename($image);
		$image_path = "http://www.coxplorebd.com/uploaded_tourist_agent_image/".$image ;

	  if(empty($tourist_agent_name) || empty($tourist_agent_details) || empty($phone)|| empty($latitude )|| empty($longitude)){
		echo '<script language="javascript">';
		echo 'alert("Some Field is Empty You Must Insert Suitable Data On It !")';
		echo '</script>';
	  }else{
		$sql = "INSERT INTO tourist_agent (tourist_agent_name,tourist_agent_details,phone,latitude,longitude,image,image_path) VALUES ('$tourist_agent_name','$tourist_agent_details','$phone','$latitude','$longitude','$image','$image_path')";
		// execute query
      mysqli_query($db, $sql);
      header ('Location:tourist_agent_list.php');
...
<div id="content">
  <form method="POST" action="tourist_agent_insert.php" enctype="multipart/form-data">

    <div class="form-group has-success">
        <label class="col-xs-2 control-label" for="inputSuccess">ATM Name</label>
        <div class="col-xs-10">
            <input type="text" id="inputSuccess" class="form-control" placeholder="ATM Name" name="tourist_agent_name">
        </div>
    </div>
    ...
    <div class="form-group has-success">
        ...
        <div class="col-xs-10">
            <textarea id="myeditor" name="tourist_agent_details" id="myeditor"></textarea>
        </div>
    </div>
    ...
    <div class="form-group has-success">
        ...
        <div class="col-xs-10">
            <input type="text" id="inputSuccess" class="form-control" placeholder="Type Longitude" name="longitude">
        </div>
    </div>
...
<script type="text/javascript">
  ...
  <center>
    <div>
      ...
      <button type="submit" class="btn btn-primary" style = "width:40%" onclick="window.location.href='tourist_agent_list.php'">ATM List</button>
  	</div>
    </center>  
