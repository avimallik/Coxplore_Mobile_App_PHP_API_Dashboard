<?php
  ...
  if (isset($_POST['upload'])) {
  	...
  	// Get text
        ...
        $hospital_details = mysqli_real_escape_string($db, html_entity_decode($_POST['hospital_details']));
	    ...
	    $target = "uploded_hospital_image/".basename($image);
		$image_path = "http://localhost/coxplore/uploded_hospital_image/".$image ;

	  if(empty($hospital_name) || empty($hospital_details) || empty($phone)|| empty($latitude )|| empty($longitude)){
		echo '<script language="javascript">';
		echo 'alert("Some Field is Empty You Must Insert Suitable Data On It !")';
		echo '</script>';
	  }else{
		$sql = "INSERT INTO hospital (hospital_name,hospital_details,phone,latitude,longitude,image,image_path) VALUES ('$hospital_name','$hospital_details','$phone','$latitude','$longitude','$image','$image_path')";
		// execute query
      mysqli_query($db, $sql);
...
<div id="content">
  <form method="POST" action="hospital_insert.php" enctype="multipart/form-data">
    ...
    <div class="form-group has-success">
        <label class="col-xs-2 control-label" for="inputSuccess">Hospital Name</label>
        <div class="col-xs-10">
            <input type="text" id="inputSuccess" class="form-control" placeholder="Type Hospital Name" name="hospital_name">
        </div>
    </div>

    <div class="form-group has-success">
        <label class="col-xs-2 control-label" for="inputSuccess">Hospital Details</label>
        <div class="col-xs-10">
            <textarea id="myeditor" name="hospital_details" id="myeditor"></textarea>
        </div>
    </div>
...
<script type="text/javascript">
  ...
  <center>
    <div>
      ...
      <button type="submit" class="btn btn-primary" style = "width:40%" onclick="window.location.href='hospital_list.php'">Hospital List</button>
    </div>
  </center>