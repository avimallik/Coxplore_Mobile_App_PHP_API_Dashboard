...
ob_start();
  ...
  if (isset($_POST['upload'])) {
  	...
  	// Get text
	    ...
	    $target = "uploaded_transport_airplane_image/".basename($image);
		$image_path = "http://www.coxplorebd.com/uploaded_transport_airplane_image/".$image ;
	  ...
	  if(empty($transport_name) || empty($transport_details)|| empty($transport_phone )|| empty($transport_website)){
		...
		$sql = "INSERT INTO transport_airplane (transport_name,transport_details,transport_website,transport_phone,image,image_path) VALUES ('$transport_name','$transport_details','$transport_website','$transport_phone','$image','$image_path')";
		// execute query
      mysqli_query($db, $sql);
      header ('Location:transport_airplane_list.php');
...
<div id="content">
  <form method="POST" action="transport_airplane_insert.php" enctype="multipart/form-data">
...
<script type="text/javascript">
  ...
  <center>
    <div>
      ...
      <button type="submit" class="btn btn-primary" style = "width:40%" onclick="window.location.href='transport_airplane_list.php'">Airplane Transport List</button>
  	</div>
    </center>  
